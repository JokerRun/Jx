...
where DateDiff(dd, createdAt, getdate()) > 365
...
where [ediAsnPackageId] in
      (select id from [JXWTSDb].[dbo].[EdiAsnPackage] where DateDiff(dd, updatedAt, getdate()) > 365);
delete
...
where DateDiff(dd, updatedAt, getdate()) > 365;
...
where [deliveryReceivedRecordId] in
      (select id from [JXWTSDb].[dbo].[DeliveryReceivedRecord] where DateDiff(dd, updatedAt, getdate()) > 365);
delete
